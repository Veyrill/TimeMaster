  ...
  items.forEach(item => {
    ...
    list.innerHTML += `
      <li class="history-item">
        <div>
          <button class="btn-icon" onclick="toggleFavorite(${firstId})">
            ${item.is_favorite ? "⭐" : "☆"}
          </button>

          <span class="entry-title">${item.activity}</span>
          <span class="tag">${item.category || "Bez kategorii"}</span>
        ...
        <div>
          ...
          <button class="btn-icon" onclick="toggleStatus(${firstId})">
            ${isRunning ? "⏳" : "✔"}
          </button>

          <button class="btn-icon" onclick="addToGoogleCalendar(${firstId})">
            📅
          </button>
...
function renderCategorySummary() {
  ...
  list.innerHTML = Object.entries(summary)
    .map(([cat, sec]) => `<li><strong>${cat}</strong>: ${formatSeconds(sec)}</li>`)
    .join("");
}

/* ===== GOOGLE CALENDAR ===== */
async function addToGoogleCalendar(entryId) {
  const res = await fetch(`/api/calendar/google/${entryId}`);
  const data = await res.json();

  if (data.url) {
    window.open(data.url, "_blank");
  } else {
    alert("Nie udało się dodać do Google Calendar");
  }
}
